...
module.exports = {
  v2SuffixTemplatesDeprecated(template, name) {
    const initCommand = 'hd-ui init ' + template.replace('-2.0', '') + ' ' + name
  ...
  v2BranchIsNowDefault(template, name) {
    const initCommand = 'hd-ui init ' + template + '#1.0' + ' ' + name

    console.log(chalk.green('  This will install Vue 2.x version of the template.'))
    console.log()
    console.log(chalk.yellow('  For Vue 1.x use: ') + chalk.green(initCommand))
    console.log()
  },
}
